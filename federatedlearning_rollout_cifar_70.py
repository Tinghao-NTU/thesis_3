...
for ep_iter in range(0, n_episodes+1):
    ...
    print(net_glob)
    net_glob.train()
    #torch.save(net_glob.state_dict(), 'parameter_cifar_nownownow.pkl')
    net_glob.load_state_dict(torch.load('parameter_cifar_nownownow.pkl'))
    w_glob = net_glob.state_dict()
    ...
    index_edge_num = []
    sele_num = int(args.num_users * 0.7)
    ...
    for roll_num in range(5):   
        ...
        while not done: 
            devices,pool = cluster_select(clusteded_dic,sele_each_num,epoch,pool = pool)
            idxs_users = [i for i in range(args.num_users)]
            ...
            file_handle=open('unreplaced_'+info+args.dataset+'_'+str(roll_num)+'.txt',mode='a')
    ...
    for roll_num in range(5):   
        ...
        w_list = copy.deepcopy(w_list_init)
        while not done: 
            devices,_ = cluster_select(clusteded_dic,sele_each_num,0)
            idxs_users = [i for i in range(args.num_users)]
            sele_devices= devices
            index_edge_users = alloc_devices(sele_devices,args.servers,index_edge_num)
            for j in range(edge_iter):
            ...
            for i in range(args.servers):
                net_glob[i].load_state_dict(w_glob)        
            acc_train, loss_train = test_img(net_glob[0], dataset_test, args)
            file_handle=open('K_means_'+info+args.dataset+'_'+str(roll_num)+'.txt',mode='a')
            file_handle.write(str(acc_train.numpy().item()))
            file_handle.write('\n')
            file_handle.close()
    ...
    for roll_num in range(5):   
        ...
        while not done: 
            idxs_users = [i for i in range(args.num_users)]
            sele_devices= np.random.permutation(idxs_users)[:sele_num]
            index_edge_users = alloc_devices(sele_devices,args.servers,index_edge_num)
